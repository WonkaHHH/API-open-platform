...
import com.xiaoxi.common.model.vo.LoginUserVO;
import com.xiaoxi.common.model.vo.UserVO;
import com.xiaoxi.xiapi.service.UserService;
import com.xiaoxi.xiapi.utils.SqlUtils;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
     ...
     * 盐值，混淆密码
     */
    private static final String SALT = "xiaoxi";
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        synchronized (userAccount.intern()) {
            ...
            // 3. 分配 accessKey与secretKey
            String accessKey = DigestUtil.md5Hex(SALT + userAccount + RandomUtil.randomString(5));
            String secretKey = DigestUtil.md5Hex(SALT + userAccount + RandomUtil.randomString(6));
            // 4. 插入数据
            User user = new User();
...
//    }
    ...
    @Override
    public DevKeyVO updateDevKey(User loginUser) {
        synchronized (loginUser.getUserAccount().intern()){
            String accessKey = DigestUtil.md5Hex(SALT + loginUser.getUserAccount() + RandomUtil.randomString(5));
            String secretKey = DigestUtil.md5Hex(SALT + loginUser.getUserAccount() + RandomUtil.randomString(6));

            loginUser.setAccessKey(accessKey);
            loginUser.setSecretKey(secretKey);

            boolean isSueccess = this.updateById(loginUser);

            if (!isSueccess){
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "更新失败");
            }

            DevKeyVO devKeyVO = new DevKeyVO();
            devKeyVO.setAccessKey(accessKey);
            devKeyVO.setSecretKey(secretKey);
            return devKeyVO;
        }
    }
}
